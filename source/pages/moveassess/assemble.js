...
Page({
  ...
  submit: function () {
    ...
    wx.showModal({
      title: '确认',
      content: '拆装费用为' + (this.data.totalamount).toString() + "元，确定提交吗？",
      success: function (res) {
        if (res.confirm) {
          ...
          if (pages.length > 1) {
            ...
            //关键在这里
            prePage.assembleselectedcallback(that.data);
          }
          wx.navigateBack();
        } else if (res.cancel) {
  ...
  if(options.length==0){
    ...
    assembleoptionApi.list({ "orderby": "seq", "status": "A" }, function (data) {
