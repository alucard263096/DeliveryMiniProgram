...
Page({
  ...
  submit: function () {
    ...
    wx.showModal({
      title: '确认',
      content: '特殊件费用为' + (this.data.totalamount).toString() + "元，确定提交吗？",
      success: function (res) {
        if (res.confirm) {
          ...
          if (pages.length > 1) {
            ...
            //关键在这里
            prePage.specialstuffselectedcallback(that.data);
          }
          wx.navigateBack();
        } else if (res.cancel) {
  ...
  onLoad: function (options) {
    ...
    if (options.length == 0) {
      ...
      specialstuffoptionApi.list({ "orderby": "seq", "status": "A" }, function (data) {
